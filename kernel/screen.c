...
void clear_screen(char *vram, int size, unsigned char c) // 15:pure white
{
	...
	{
		write_mem8(vram + i, c); // if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
	}
}
...
void color_screen(char *vram, int size) // 15:pure white
{
	for (int i = 0; i < size; i++)
	{
		write_mem8(vram + i, i & 0x0f); // if we write 15 ,all pixels color will be white,15 mens pure white ,so the screen changes into white
	}
}
