  ...
  while (1) {
    ...
    if (fifo8_status(&keyfifo) + fifo8_status(&mousefifo) == 0) {
      ...
      if (fifo8_status(&keyfifo) != 0) {
        boxfill8(binfo->vram, binfo->scrnx, COL8_840000, 0, 16, 31,
                 31);  // chriszz添加。刷字符区域为蓝色。
        d = fifo8_get(&keyfifo);
        io_sti();
        sprintf(s, "%x", d);
      ...
      if (fifo8_status(&mousefifo) != 0) {
        // boxfill8(binfo->vram, binfo->scrnx, COL8_000084, 32, 16, 64,
        // 31);//chriszz添加。刷字符区域为蓝色。
        d = fifo8_get(&mousefifo);
        io_sti();
        if (mouse_decode(&mdec, d) != 0) {
          putblock8_8(binfo->vram, binfo->scrnx, 12, 19, mx, my, background,
                      12);  // 刷[原]鼠标位置
          ...
          for (int y = 0; y < 19; y++) {
            for (int x = 0; x < 12; x++) {
              background[y * 12 + x] =
                  binfo->vram[(my + y) * binfo->scrnx + (mx + x)];
            }
          }

          putblock8_8(binfo->vram, binfo->scrnx, 12, 19, mx, my, mcursor,
                      12);  // 刷[新]鼠标位置
        }
      }
    }
